        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div id="schedule-warning" class="mb-6 hidden">
                <div class="conflict-warning">
                    ...
                    <p class="text-red-600 mt-2 font-medium">Admin tidak dapat menyetujui peminjaman ini karena ada
                        konflik jadwal.</p>
                </div>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function checkScheduleConflict() {
                    fetch(
                            `/peminjaman/availability?ruangan_id=${peminjamanData.ruangan_id}&tanggal=${peminjamanData.tanggal}`
                            )
                ...
                function updateRoomSchedule(events) {
                    ...
                    if (events.length === 0) {
                        html =
                            '<div class="text-green-600"><strong>✅ Tidak ada jadwal lain</strong><br><small>Ruangan kosong pada tanggal ini</small></div>';
                    } else {
                        html = '<div class="mb-2"><strong>Jadwal Ruangan pada Tanggal Ini:</strong></div>';

                        events.forEach(event => {
                            const statusClass = event.status === 'disetujui' ? 'text-red-600' :
                                'text-yellow-600';
                        ...
                        html +=
                            '<strong>⚠️ Perhatian:</strong> Ruangan sudah dipinjam pada tanggal ini. Hanya satu peminjaman per hari yang diizinkan.';
                        html += '</div>';
                    }
                ...
                function showConflictWarning(conflicts) {
                    ...
                    conflicts.forEach(conflict => {
                        const statusText = conflict.status === 'disetujui' ? 'Sudah Disetujui' :
